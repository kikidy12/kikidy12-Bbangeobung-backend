...
public class StoreV2Controller {
    ...
    public ResponseDto<List<V2StoreDto.V2StoreRes>> getMyStores(
            @Parameter(hidden = true) @AuthenticationPrincipal UserDetailsImpl userDetails
    ) {
        return ResponseDto.of(HttpStatus.OK, "조회 성공", storeService.getStoreByMe(userDetails.getUser()));
    }
    ...
    public ResponseDto<V2StoreDto.V2StoreRes> addStore(
            @ModelAttribute V2StoreDto.V2StoreAdd dto,
            @Parameter(hidden = true) @AuthenticationPrincipal UserDetailsImpl userDetails
    ) {
